...
**기록 형식 (2026-09-06부터 적용, `#20` 이후 항목부터):** 증상 → 원인 → **검토한 대안** → 해결 방법 순으로 씁니다.
"검토한 대안"엔 채택 안 한 방법과 왜 안 골랐는지(성능/호환성/범위 등 근거)를 짧게라도 남깁니다 — Part 3(OCR 기능 구현 기록)의 "해결 방안 검토" 방식을 트러블슈팅 항목에도 적용한 것. `#18`~`#20`은 이 형식 이전에 쓰여서 소급 적용 안 함.
